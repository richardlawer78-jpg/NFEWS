...
const Charts = () => {
  ...
  if (loading) return (
    <div className="page-content">
      <Navbar title="Water Level Charts" subtitle="Real-time trend analysis" />
          ...
          <div className="chart-wrapper">
            <ResponsiveContainer width="100%" height={340}>
              <AreaChart data={currentReadings} margin={{ top: 20, right: 30, left: 10, bottom: 10 }}>
                ...
                <YAxis
                  tick={{ fontSize: 11, fill: '#94a3b8' }}
                  domain={[0, Math.max(threshold + 0.5, 2.5)]}
                  tickFormatter={v => `${v}m`}
                  axisLine={false}
                  tickLine={false}
                />
                ...
                <ReferenceLine
                  ...
                  strokeDasharray="6 3"
                  strokeWidth={2}
                  label={{ value: `🚨 Critical ${threshold}m`, fill: '#dc2626', fontSize: 12, fontWeight: 700 }}
                />
                <ReferenceLine
                  y={threshold * 0.85}
                  stroke="#f59e0b"
                  strokeDasharray="4 4"
                  label={{ value: `⚠ Danger ${(threshold * 0.85).toFixed(2)}m`, fill: '#f59e0b', fontSize: 11 }}
                />
                <ReferenceLine
                  y={threshold * 0.65}
                  stroke="#22c55e"
                  strokeDasharray="4 4"
                  label={{ value: `⚡ Caution ${(threshold * 0.65).toFixed(2)}m`, fill: '#22c55e', fontSize: 11 }}
                />
                <Area
                  type="monotone"
                  dataKey="level"
                  stroke={currentDistrict?.color}
                  strokeWidth={3}
                  fill="url(#levelGrad)"
                  name="Water Level"
                  dot={{ fill: currentDistrict?.color, r: 4, strokeWidth: 2, stroke: '#fff' }}
                  activeDot={{ r: 7 }}
                />
              </AreaChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart margin={{ top: 10, right: 30, left: 10, bottom: 10 }}>
                ...
                {DISTRICTS.map(d => (
                  <Line
                    key={d.id}
                    data={(allReadings[d.id] || []).slice(-10)}
                    type="monotone"
                    dataKey="level"
                    stroke={d.color}
                    strokeWidth={2.5}
                    name={d.name}
                    dot={{ r: 4, fill: d.color, stroke: '#fff', strokeWidth: 2 }}
                    activeDot={{ r: 6 }}
                  />
                ))}
              </LineChart>
